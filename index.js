// Берете массив goods. 
// Показываете на экране товары. 
// Добавляете ко всем элеметам имени атрибут id со значением имен из объетов.
// потом по значению id удаляете.
...
let li1=Array.from(li)
  let asking=prompt('Введите название товара для удаления')
  let a=li1.filter((item)=>item.id !==asking)
  ol.innerHTML=''
  a.forEach(item=>ol.appendChild(item))
  }
